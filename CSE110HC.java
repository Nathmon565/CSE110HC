			...
			switch (status) {
				case menu:
					...
					int choice = IntInputList(new String[] { "Start Game", "Set Seed", "Set Difficulty",
							"Set Global Text Speed Modifier", "Enable Tutorial", "Quit" }, 0);
					switch (choice) {
						case 1:
							if (tutorial == null) {
				...
				case tavern:
					...
					switch (tavernChoice) {
						...
						case 4:
							...
							break;
						case 5:
							if (favors >= 3 && !Player.HasDungeonMap()) {
								Player.AddFavors(-3);
	...
	private boolean MoveTo(Vector2 coord) {
		...
		if (room.IsDoor()) {
			...
			if (!Player.HasKey() && !Player.HasSkeletonKey()) {
				...
				if (Player.HasKey()) {
					print("You have the matching key. Would you like to unlock the door?");
					...
					print("The key you purchased seems different from before...");
...
class TutorialHandler {
	...
	public void PrintTutorial(Tutorials tutorial) {
		...
		switch (tutorial) {
			...
			case favor:
				CSE110HC.print(
						"Favors are one-time use items that will help you in your journey. They range from thinning the number of enemies on the next floor to a strange map which will reveal the next dungeon's floor. These favors cannot stack, but are powerful and essential in getting to the deeper depths of the dungeon...");
				break;
			case nextFloor:
				CSE110HC.print(
